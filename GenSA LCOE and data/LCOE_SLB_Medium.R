...
#  data <- read_excel('Dados_medium.xlsx')
  ...
  x <- round(x,0) # transforma em inteiros
  ...
  Dem_TDG <- x[4] # Demanda contratada de geração em kW - variável móvel para o mínimo
  REC <- 0.8 # Porcentagem do SoH que pode recarregar com energia da rede em horário fp  
  
  ### Para testar ###
#    Q_pv <- 500 # Quantidade de painéis instalados - Variável para otimização
#    Q_mod <- 250 # Quantidade de baterias novas instaladas - Variável para otimização
#    Dem_TDC_fp <- 64 # Demanda contratada fora ponta em kW - variável móvel para o mínimo
#    Dem_TDC_p <- Dem_TDC_fp # Demanda contratada na ponta em kW - variável móvel para o mínimo
#    Dem_TDG <- 300 # Demanda contratada de geração em kW - variável móvel para o mínimo
#    REC <- 0.8 # Porcentagem do SoH que pode recarregar com energia da rede em horário fp
