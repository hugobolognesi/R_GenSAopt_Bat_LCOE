  ...
  # x <- c(500, 0, 110, 70, 200)
  
  Q_pv <- 0 # Quantidade de painéis instalados - Variável para otimização
  ...
  Dem_TDC_p <- 43 # Demanda contratada na ponta em kW - variável móvel para o mínimo
  Dem_TDG <- 0 # Demanda contratada de geração em kW - variável móvel para o mínimo
  ...
  tx_r <- 0.0 # Taxa de retorno estimada
  ...
  Emi_pv <- 1997/1000 #Emissoes de CO2 em kg CO2eq/kWp 
  ...
  Co2 <- ((T_yrl$P_gl_c+T_yrl$P_gb_c) * Emi_BR*(1+perdas) #Emissoes por uso de energia da rede
          + (SoHInicial/EO1L)*(EO1L-EUL*EO1L)*BESS  #Emissoes por uso da bateria
          + T_yrl$P_pv_c*Emi_pv )
