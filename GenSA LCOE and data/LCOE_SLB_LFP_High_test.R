...
cat("INICIO DA FUNCAO OBJETIVO \n")
  ...
  Dados_0 <- read_excel('Dados_0.xlsx')
  
  Dia <- Dados_0$Dia
  mes <- Dados_0$mes
  hora <- Dados_0$hora
  hora_p <- Dados_0$hora_p
  Irradiacao <- Dados_0$Irradiacao
  CicloCorrido <- Dados_0$CicloCorrido
  Demanda_h <- Dados_0$Demanda_h

  sens <- 0.9 # Variaçao para sensibilidade
  
  ### Testes
  Q_pv <- 647 # Quantidade de painéis instalados - Variável para otimização
  Q_mod <- 272 # Quantidade de baterias novas instaladas - Variável para otimização
  Dem_TDC_fp <- 47 # Demanda contratada fora ponta em kW - variável móvel para o mínimo
  Dem_TDC_p <- 44 # Demanda contratada na ponta em kW - variável móvel para o mínimo
  Dem_TDG <- 200 # Demanda contratada de geração em kW - variável móvel para o mínimo
  ...
  EO1L <- 0.70 # SoH no fim da primeira vida
  T1l <- 7300 # Ciclos na primeira vida
  ...
  EUL <- 0.6 # Porcentagem do SoH que define o fim da vida util
  ...
  P_preco <- 0.65 #Poercentagem do preço da bateria nova
  ...
  Ef_pv_i <- 0.18 # Eficiência de conversão da irradiação em corrente DC dos painéis FV - Fonte: BYD+Homer
  ...
  TUSD_p <- 110.49/1000 # Tarifa de Uso do Sistema de Distribuiçao em R$/kWh
  TD_C_fp <- 16.66 # Tarifa de Demanda do Sistema de Distribuiçao em R$/kW
  TD_C_p <- 47.86 # Tarifa de Demanda do Sistema de Distribuiçao em R$/kW
  ...
  Impostos <- sens*1 / (1 - PIS - COFINS) / (1 - ICMS) # Calculo de impostos incidentes
  ...
  repeat {
    ...
    if (i + 1 > length(CicloCorrido)) {
      if (Q_mod > 0) {
        ...
        if (SoH[c + 1]/(SoHInicial/EO1L) < EUL || SoH[c + 1] * (SoC_max - SoC_min) < E_p) {
  ...
  C_dem <- C_DEM_fp + C_DEM_p + C_DEM_G
  ...
  Tb_custos <- data.frame(T_yrl[, 1], CAPEX_pv, CAPEX_bat, C_OM_pv, C_OM_bat, #Custos de equipamentos
                           C_energy_in, C_energy_out, C_pen,  #Custos variáveis C_energy_yrl,
                          C_DEM_fp, C_DEM_p, C_DEM_G) #Custos fixos
  ...
  Emi_BR <- 61.7/1000 #Emissões de CO2 em kg CO2eq/kWh Fonte: BEN 2023 (dados 2022)
  perdas <- 0.15 #Perda de energia na rede Fonte: BEN 2023
  Emi_pv <- 749.8/1000 #Emissoes de CO2 em kg CO2eq/Wp instalado fonte Parisini
  Emi_LFP <- 73.5 #Emissoes de CO2 para fabriar a bateria em kg CO2eq/kWh de bateria Fonte: Bouter 2022
  
  Co2 <- ((T_yrl$P_gl_c+T_yrl$P_gb_c) * Emi_BR*(1+perdas) #Emissoes por uso de energia da rede
          + (SoHInicial/EO1L)*(EO1L-EUL*EO1L)*BESS*Emi_LFP)  #Emissoes por uso da bateria
  ...
  ggplot(tb_plot, aes(x = T_yrl...1., y= value, fill = variable)) +
    ...
    labs(title = "", x = "Year", y = "BRL") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  
  Tb_h <- data.frame(mes, Dia, hora, Irradiacao, Demanda_h, 
                     P_pv_h, P_pl_h, P_pb_h,P_bl_h, P_gb_h, P_gl_h, P_pg_h, SoC_h)
