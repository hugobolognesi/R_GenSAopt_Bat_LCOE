...
# Função objetivo para calcular o LCOE do sistema com PV+SLBESS de alta demanda
LCOE_SLB_LFP_High <- function(x) {
...
#  Dados_0 <- read_excel('Dados_0.xlsx')
  ...
  Q_pv <- x[1] # Quantidade de painéis instalados - Variável para otimização
  Q_mod <- x[2] # Quantidade de baterias novas instaladas - Variável para otimização
  Dem_TDC_fp <- x[3] # Demanda contratada fora ponta em kW - variável móvel para o mínimo
  Dem_TDC_p <- x[4] # Demanda contratada na ponta em kW - variável móvel para o mínimo
  Dem_TDG <- x[5] # Demanda contratada de geração em kW - variável móvel para o mínimo
  ...
  EO1L <- 0.65 # SoH no fim da primeira vida
  ...
  ### Testes
#  Q_pv <- 621 # Quantidade de painéis instalados - Variável para otimização
#  Q_mod <- 0 # Quantidade de baterias novas instaladas - Variável para otimização
#  Dem_TDC_fp <- 105 # Demanda contratada fora ponta em kW - variável móvel para o mínimo
#  Dem_TDC_p <- 51 # Demanda contratada na ponta em kW - variável móvel para o mínimo
#  Dem_TDG <- 192 # Demanda contratada de geração em kW - variável móvel para o mínimo
#  REC <- 0.8 # Porcentagem do SoH que pode recarregar com energia da rede em horário fp
