...
cat("INICIO DA FUNCAO OBJETIVO \n")
  ...
  ### Testes
  Q_pv <- 632 # Quantidade de painéis instalados - Variável para otimização
  Q_mod <- 259 # Quantidade de baterias novas instaladas - Variável para otimização
  Dem_TDC_fp <- 46 # Demanda contratada fora ponta em kW - variável móvel para o mínimo
  Dem_TDC_p <- 44 # Demanda contratada na ponta em kW - variável móvel para o mínimo
  Dem_TDG <- 195 # Demanda contratada de geração em kW - variável móvel para o mínimo
  ...
  EUL <- 0.75 # Porcentagem do SoH que define o fim da vida util
  ...
  Emi_LMO <- 74.1 #Emissoes de CO2 para fabriar a bateria em kg CO2eq/kWh de bateria
  
  Co2 <- ((T_yrl$P_gl_c+T_yrl$P_gb_c) * Emi_BR*(1+perdas) #Emissoes por uso de energia da rede
          + (SoHInicial/EO1L)*(EO1L-EUL*EO1L)*BESS*Emi_LMO)  #Emissoes por uso da bateria
