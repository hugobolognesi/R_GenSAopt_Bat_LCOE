  ...
  sens <- 1.1 # Variaçao para sensibilidade
    ### Para testar ###
    Q_pv <- 609 # Quantidade de painéis instalados - Variável para otimização
    Q_mod <- 200 # Quantidade de baterias novas instaladas - Variável para otimização
    ...
    Dem_TDG <- 201 # Demanda contratada de geração em kW - variável móvel para o mínimo
    ...
    EO1L <- 0.7 # SoH no fim da primeira vida
    ...
    # Parâmetros de tempo
  anos_t <- 10 # Quantidade de anos para o sistema
  ...
  TE_fp <- sens*302.56/1000 # Tarifa de energia em R$/kWh
  TE_p <- sens*484.11/1000 # Tarifa de energia em R$/kWh
  TUSD_fp <- sens*110.49/1000 # Tarifa de Uso do Sistema de Distribuiçao em R$/kWh
  TUSD_p <- sens*1273.64/1000 # Tarifa de Uso do Sistema de Distribuiçao em R$/kWh
...
#  C_DEM_p[1] <- 0
  ...
  Tb_custos <- data.frame(T_yrl[, 1], CAPEX_pv, CAPEX_bat, C_OM_pv, C_OM_bat, #Custos de equipamentos
                          C_energy_in, C_energy_out, C_pen,  #Custos variávei que somam C_energy_yrl,
